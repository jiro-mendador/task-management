...
function insertProject($project, $pdo)
{
  $sql = "INSERT INTO projects 
            (user_id, title, description, deadline, priority_level, progress, date_added, date_completed)
          VALUES 
            (:user, :title, :descrip, :deadline, :prio, :progress, :added, DEFAULT)
          RETURNING id;";
  ...
  if ($statement->execute()) {
    ...
    echo json_encode(array("res" => $result["id"]));
  }
}
...
function getAllProjects($project, $pdo)
{
  ...
  if ($project->sortBy !== "priority_level") {
    $sql .= " ORDER BY " . preg_replace('/[^a-zA-Z_]/', '', $project->sortBy) . " " . strtoupper($project->ascDesc);
  } else {
    $sql .= " ORDER BY CASE priority_level 
                WHEN 'High' THEN 1 
                WHEN 'Medium' THEN 2 
                WHEN 'Low' THEN 3 
              END " . strtoupper($project->ascDesc) . ", deadline " . strtoupper($project->ascDesc);
  }

  if ($project->limit !== "all") {
    $sql .= " LIMIT " . intval($project->limit);
  }
